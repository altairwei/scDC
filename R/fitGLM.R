...
fitGLM <- function(res, condition, subject_effect = TRUE, pairwise = TRUE, fixed_only = FALSE, verbose = TRUE){
  ...
  for(i in 1:ncol(res$nstar)){
    ...
    mod <- stats::glm(
      cell_count ~ cellTypes + cond + cellTypes:cond + subject,
    ...
    if (subject_effect) {
      # No difference between `pairwise=TRUE/FALSE`
      if(pairwise){
        fit_fixed[[i]] <- mod

        if(!fixed_only){
          fit_random[[i]] <- lme4::glmer(
            cell_count ~ cellTypes + cond + cellTypes:cond + (1 | subject ),
            data = glm_df, family = poisson(link=log), control = glmerControl(nAGQ = 0L)
          )
        }
      } else {
        fit_fixed[[i]] <- mod
      ...
      fit_fixed[[i]] <- stats::glm(
        cell_count ~ cellTypes + cond +  cellTypes:cond,
        data = glm_df, family = poisson(link=log)
      )
    }
  }
